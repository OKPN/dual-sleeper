...
11. **大容量ダウンロード / 高トラフィック完了時の自動通知**
    * 10分以上続いていた高通信が収束した際、離席消灯中 (State 2) であればスマホ（Discord / Telegram）へダウンロード完了通知を送信します。
12. **AI生成・処理完了時の自動通知 (Windowsトースト ＆ スマホ)**
    * ComfyUI、SD-WebUI、PyTorch 等の 30秒以上の重い生成タスクが終了した際、PC作業中であれば画面右下に「ピコン♪」とトースト通知を出し、離席中であればスマホへ完了通知を即座に知らせます。
...
| | `ai_completion_notification.enabled` | `false` | bool | AI生成・処理完了時の自動通知（初期無効）。 |
| | `ai_completion_notification.min_duration_seconds` | `30` | 秒 | AI完了通知の対象とする最小処理継続時間（30秒）。 |
| | `ai_completion_notification.trigger_condition` | `"always"` | 文字列 | 通知条件（`"always"` 常時, `"state2_only"` 離席中のみ）。 |
| | `ai_completion_notification.desktop_toast` | `true` | bool | Windows画面右下ポップアップ(トースト)通知を表示するか。 |
